...

{
    ...
    public function view_ekspedisi($id_tujuan) {
         ...
         $response = DB::select(DB::raw("SELECT * FROM `penumpangs` as p LEFT JOIN (SELECT id_penumpang, COUNT(id_barang) as total FROM `barangs` GROUP BY id_penumpang) as b ON p.id_penumpang = b.id_penumpang where p.id_tujuan = 5 AND DATE_FORMAT(p.tanggal , '%Y-%m-%d') = CURDATE() AND p.id_penumpang NOT IN (SELECT id_penumpang FROM `keberangkatans`)"));
    ...
    public function get_master_barang() {
         ...
         $response = DB::select(DB::raw("select * from master_barang"));
...
//        }
    ...
    public function view_harian(Request $request, $id_armada) {
        ...
        if($request->input('tanggal') == null) {
            ...
            $penumpang = DB::select(DB::raw("SELECT jk.id_jadwal, jk.jadwal, jk.harga, jk.status, jk.ekstra, d.nama_dermaga as tujuan_awal , 
...

    {
        ...
        $penumpangs = DB::select(DB::raw("SELECT p.*, jp.nama_jns_penum, d.nama_dermaga as tujuan from keberangkatans k
...

    {
        ...
        $keberangkatans = DB::select(DB::raw("SELECT DATE(k.tanggal_keberangkatan) AS tanggal, d1.nama_dermaga AS dermaga_awal, d2.nama_dermaga AS dermaga_akhir, jk.jadwal, kp.nama_kapal, COUNT(k.id) AS jumlah FROM keberangkatans k
    ...
    public function updateInvoiceCallback(Request $request){
        ...
        if ($hasher == $request->hashcodeKey){
            $invoice = Invoice::where('bill_number', $bill_number)->firstOrFail();
            $penumpangs = DB::table('penumpangs as p')
                        ->select('p.*')
                        ->join('keberangkatans as k', 'k.id_penumpang', 'p.id_penumpang')
            ...
            return response()->json(['message' => 'HashcodeKey salah.'], 500);
        }
    ...
    // private function checkSisaSlot($request)

    // {

    //     $penump_per_tiket = array();
