...
class DashboardController extends Controller
{
    ...
    public function detail(Request $request, $id_jadwal) {
        ...
        $penumpang_unverif = DB::select(DB::raw(
    ...
    public function detailTanpaHistory(Request $request, $id_jadwal) {
        ...
        $data['jenis'] = DB::select(DB::raw("SELECT jp.id_jns_penum, jp.nama_jns_penum, kp.total FROM `jenis_penumpangs` as jp LEFT JOIN ( SELECT p.id_jns_penum, COUNT(p.id_jns_penum) as total FROM `penumpangs` as p INNER JOIN `keberangkatans` as k ON p.id_penumpang = k.id_penumpang WHERE DATE_FORMAT(k.tanggal_keberangkatan , '%Y-%m-%d') = '$tanggal' AND k.id_jadwal = '$id_jadwal' GROUP BY p.id_jns_penum ) as kp ON jp.id_jns_penum = kp.id_jns_penum"));
    ...
    public function detailHarian(Request $request)
    {
        ...
        $details = DB::select(DB::raw("SELECT kp.grt, kp.panjang, kp.dwt,hk.id_kapal,hk.id_jadwal,r.tujuan_awal,r.tujuan_akhir,kp.*, a.nama_armada, 
    ...
    public function detailBulanan(Request $request)
    {
        ...
        $details = DB::select(DB::raw("SELECT kp.grt, kp.panjang, kp.dwt,hk.id_kapal,hk.`id_jadwal`,r.tujuan_awal,r.tujuan_akhir,kp.nama_kapal, a.nama_armada, 
    ...
    public function detailBulananPDF(Request $request)
    {
        ...
        $data['details'] = DB::select(DB::raw("SELECT kp.grt, kp.panjang, kp.dwt,hk.id_kapal,hk.`id_jadwal`,
    ...
    public function detailTahunan(Request $request)
    {
        ...
        $details = DB::select(DB::raw("SELECT MONTH(hk.tanggal_berangkat) as bulan,a.id_armada,a.nama_armada,r.tujuan_awal,r.tujuan_akhir,
            count(hk.id) as count_trip,sum(hk.jml_penumpang) as jml_penumpang FROM history_keberangkatan hk
            INNER JOIN jadwal_keberangkatans jk ON hk.`id_jadwal`=jk.`id_jadwal`
            inner join rutes r on r.id_rute=jk.id_rute
            INNER JOIN kapals kp ON kp.`id_kapal`=hk.`id_kapal`
    ...
    {
        $jadwal = DB::select(DB::raw("SELECT jk.*, a.nama_armada, n.nama_nahkoda, kp.nama_kapal, kp.id_kapal, d1.lokasi as lokasi_awal, d1.nama_dermaga as dermaga_awal, d1.lokasi as lokasi_akhir, d2.nama_dermaga as dermaga_akhir,
