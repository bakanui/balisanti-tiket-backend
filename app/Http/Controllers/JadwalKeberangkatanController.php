...
class JadwalKeberangkatanController extends Controller
{
    ...
    public function get_all(Request $request)
    {
        if (isset($request->nama_armada)) {
            ...
            $jadwal = DB::select(DB::raw("SELECT a.nama_armada,jk.id_jadwal,jk.jadwal,kp.nama_kapal, kp.kapasitas_penumpang, nk.nama_nahkoda, jk.status,d.nama_dermaga as tujuan_awal, 
    ...
    public function cariDermagaTujuan()
    {
        if (isset($_GET['awal'])) {
            $id_awal = $_GET['awal'];
            $data = DB::select(DB::raw("SELECT DISTINCT(r.tujuan_akhir), d.nama_dermaga FROM rutes r 
    ...
    public function listJadwalDariRute(Request $request)
    {
        ...
        if ($request->tanggal > date('Y-m-d')) {
            if (isset($_GET['id_armada'])) {
                $id_armada = $_GET['id_armada'];
                $data = DB::select(DB::raw("SELECT jk.id_jadwal,jk.jadwal, jk.id_armada, a.nama_armada, n.nama_nahkoda, k.nama_kapal, k.kapasitas_penumpang, l.nama_loket, IFNULL(x.total_penumpang,0) as total_penumpang FROM jadwal_keberangkatans jk 
    ...
    public function cariZonaTujuan()
    {
        if (isset($_GET['zona_awal'])) {
            ...
            $data = DB::select(DB::raw("SELECT z.id_zona, z.lokasi FROM rutes r 
                        INNER JOIN dermagas d ON r.tujuan_awal = d.id_dermaga
                        LEFT JOIN dermagas d2 ON d2.id_dermaga = r.tujuan_akhir
                        INNER JOIN zona z ON z.id_zona = d2.id_zona
                        WHERE d.id_zona = $id_awal GROUP BY d2.id_zona"));
